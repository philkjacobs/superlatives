...
return(<div className="custom-button waitingroom">{player}</div>)
  ...
  login(type){
    ...
    switch(type){
      case 'host':
        this.setState({
          socket: new WebSocket(`wss://${location.host}/ws?name=${this.state.playerName}`)
        })
        break;
      case 'join':
        this.setState({
          socket: new WebSocket(`wss://${location.host}/ws?name=${this.state.playerName}&game=${this.state.gameId}`)
        })
        break;
      default:
